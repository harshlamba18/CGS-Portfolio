...
export default function EmptyRoom({
  ...
  const focusFromPcGroup = (groupRef, cameraOffset = [0, 1.2, 2.8], targetOffset = [0, 0.8, 0]) => {
    if (!onFocusSpot || !groupRef?.current) return;

    const target = groupRef.current.localToWorld(new THREE.Vector3(...targetOffset));
    const cameraPos = groupRef.current.localToWorld(new THREE.Vector3(...cameraOffset));
  ...
  return (
    <group position={position} rotation={rotation} scale={scale} {...props}>
      ...
      >
        ...
        >
          Games
          <meshStandardMaterial attach="material" emissive={'#7BFFEE'} emissiveIntensity={2} toneMapped={false} />
        </Text>
        <ScreenLED position={[-3.5, 1.5, -1.7]} rotation={[0, 2*Math.PI, 0]} scale={0.28} />
        <pointLight intensity={30} distance={8} decay={2} color={0xffffff} position={[0, 2.2, 0]} />
      ...
      >
        ...
        >
          Projects
          <meshStandardMaterial attach="material" emissive={'#FF6BCB'} emissiveIntensity={2} toneMapped={false} />
        </Text>
      ...
      <group name="glassWallGroup" position={[1.2, 0.8, 0]}>
        ...
        <Macbook position={[-1.5, 0.15, -0.5]} rotation={[0, 3*Math.PI/2, 0]} scale={2} />
      </group>
